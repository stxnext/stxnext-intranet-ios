...
- (void)setTooltipVisible:(BOOL)tooltipVisible animated:(BOOL)animated atTouchPoint:(CGPoint)touchPoint
{
    ...
    if (!self.tooltipView)
    {
        ...
        [self.chartView.headerView addSubview:self.tooltipView];
    }
    ...
    if (!self.tooltipTipView)
    {
        ...
        [self.chartView.headerView addSubview:self.tooltipTipView];
    }
    ...
    dispatch_block_t adjustTooltipPosition = ^{
        ...
        if (chartView)
        {
            ...
            self.tooltipView.frame = CGRectMake(convertedTouchPoint.x - ceil(self.tooltipView.frame.size.width * 0.5),
                                                0,//CGRectGetMaxY(chartView.headerView.frame) + self.tooltipView.frame.size.height,
